$(document).ready(function () {
    ...
    document.addEventListener("transmitHideMenu", (e) => {
...
function createMobileNavBarBaseOnExistingMenu() {
    ...
    const innerMenuWrapper = (element) =>
        `<ul class="sub-menu-items mobile">${element}</ul>`;
    const menuCollapseDiv = (element) =>
        `<div class="mobile-option menu">${innerMenuWrapper(element)}</div>`;
    ...
    if (scrapeExistingMenuNode.length) {
        const shapeMenu = scrapeExistingMenuNode
            .map(
                (_menu) => `<li>
            ...
            .join("");
        items[1].insertAdjacentHTML(
            "beforebegin",
            `${menuDiv}${menuCollapseDiv(shapeMenu)}`
        );
    }
}
