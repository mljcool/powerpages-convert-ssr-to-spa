...
const removeDupeSrcLinks = [
	...
	'css/font-awesome.BootstrapV5.bundle.css',
	'preform.BootstrapV5.bundle.css',
	'bootstrap.min.css',
	'Common/js/MsdynIcons.js',
	'Common/js/Utility.js',
	'@fluentui/web-components',
	'Common/js/NavigationControl.js',
...
function testPages() {
	const testPages = [
		{
			pathURL: '/RFQ-list/RFQ-list.html',
		},
		{
			pathURL: '/Purchase Order List/Purchase Order List.html',
		},
		{
			pathURL: '/Vendor Information/Vender Information.html',
		},
	];
...
document.addEventListener('DOMContentLoaded', function () {
	...
	allsubmenus.forEach((_submenus) => {
		...
		_submenus.addEventListener('click', function (e) {
			...
			if (storeTestPages.length && decodeURL !== currentURL) {
				const searchPage = storeTestPages.find(
					(_link) => _link.route === decodeURL
				);
				window.history.pushState({}, '', anchor.href);
				console.log('searchPage', searchPage);
				pageRenderer(searchPage.htmlPage);
				currentURL = decodeURL;
			}
		});
	});
});
